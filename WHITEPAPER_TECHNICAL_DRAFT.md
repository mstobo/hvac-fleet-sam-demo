...
## The Query Plane: Efficient by Default, Thorough on Demand

The LLM is invoked only when a human asks a question. In other words, AI is a query-plane subscriber to curated events, not part of the event ingestion path. But even here, we learned efficiency matters.
...
The key EDA point: the same curated event stream can fan out in parallel to dashboards, alert workflows, and SAM query consumers without coupling those consumers to each other.
...
Treat the two-plane model as event refinement, not architectural drift:
- Data plane refines and routes events (EDA core)
- Query plane consumes refined events for human-facing reasoning (optional)
...
- **LLM endpoint**: Any OpenAI-compatible provider or local model via LiteLLM
