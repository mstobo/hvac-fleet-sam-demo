...
log = config.get_logger("AnalysisSlackBridge")
...
def on_connect(client: mqtt.Client, _userdata, _flags, reason_code, _properties=None) -> None:
    if reason_code == 0:
        log.info("connected to broker %s:%s", config.BROKER_HOST, config.BROKER_PORT)
        client.subscribe(RESPONSE_TOPIC, qos=1)
        client.subscribe(ERROR_TOPIC, qos=1)
        log.info("subscribed to %s and %s", RESPONSE_TOPIC, ERROR_TOPIC)
        return
...
def main() -> int:
    ...
    log.info("starting | slack=%s response=%s error=%s", SLACK_CHANNEL, RESPONSE_TOPIC, ERROR_TOPIC)
